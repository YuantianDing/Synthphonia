...
use crate::expr::Expr;
use crate::galloc::{AllocForExactSizeIter, AllocForIter, AllocForStr, TryAllocForExactSizeIter};
...
/// 
pub struct Filter(pub Option<&'static Expr>);
...
impl Filter {
    ...
    /// 
    /// It returns a static string slice representing the operation's name within the context of the Synthphonia module, specifically identifying the `list.filter` functionality. 
...
impl std::fmt::Display for Filter {
    ...
    /// 
    /// The function checks if the `Map` contains an `Expr` and, if so, formats the output to include the expression with a "list.filter" prefix decorated by the expression's debug representation. 
    /// If no expression is present, the output simply includes "list.filter". 
...
impl crate::forward::enumeration::Enumerator1 for Filter {
...
impl crate::expr::ops::Op1 for Filter {
    ...
    fn try_eval(&self, a1: Value) -> (bool, Value) {
        ...
        if let Value::ListStr(a) = a1 {
            let a = a.iter().map(|&x| {
                ...
                let fresult = e.eval(&ctx).to_bool();
                x.iter().zip(fresult.iter()).flat_map(|(c, b)| {
