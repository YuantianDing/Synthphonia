...
use crate::{expr::{cfg::Cfg, context::Context, Expr}, value::{Type, Value}};
    ...
    pub fn new(cfg: & Cfg, ctx: & Context) -> Vec<Self> {
        ...
        cfg.iter().map(|nt| {
            ...
            Self {
                ...
                // to: TextObjData::new(cfg, ctx)
            }
        }).collect_vec()
    }
    #[inline(always)]
    pub fn update(&self, e: Expr, v: Value) -> Result<Option<&'static Expr>, ()> {
        if let Some(e) = self.all_eq.set(v, e)? {
            self.substr.update(v)?;
            self.listsubseq.update(v)?;
            if let Some(l) = &self.len { l.update(v)? };
            // self.to.update(nt, size, e, v);
            Ok(Some(e))
        } else {
            Ok(None)
        }
